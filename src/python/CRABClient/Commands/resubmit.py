...
class resubmit(SubCommand):
    ...
    def __call__(self):
        ...
        self.logger.debug('Requesting resubmission for failed jobs in task %s' % self.requestname )
        ...
        dictresult, status, reason = server.post(self.uri, data = urlencode({ 'workflow' : self.requestname}) + \
