...
class request_type(SubCommand):
    ...
    def __call__(self):
        ...
        self.logger.debug('Looking type for task %s' % self.requestname)
        dictresult, status, reason = server.get(self.uri, data = {'workflow': self.requestname, 'subresource': 'type'})
