...
class kill(SubCommand):
    ...
    def __call__(self):
        ...
        self.logger.debug('Killing task %s' % self.requestname)
        dictresult, status, reason = server.delete(self.uri, data = urlencode({ 'workflow' : self.requestname}) + '&' + urlencode(self.jobids))
        self.logger.debug("Result: %s" % dictresult)

        if status != 200:
            msg = "Problem killing task %s:\ninput:%s\noutput:%s\nreason:%s" % \
                    (self.requestname, str(self.requestname), str(dictresult), str(reason))
